...
INVALID_CPF_LIST_WITH_MASK = [
    '968.982.247-25',
    '654.063.508-93',
    '410.457.138-36',
    '111.111.111-11',
]
INVALID_CPF_LIST_WITHOUT_MASK = [
    '96898224725',
    '65406350932',
    '41045718366',
    '11111111111',
...
def test_clean_cpf():
    clened_cpf = validate_cpf.__clean(INVALID_CPF_WITH_MASK)
    assert all(i.isdigit() == True for i in clened_cpf)
    clened_cpf = validate_cpf.__clean(VALID_CPF_WITH_MASK)
    assert all(i.isdigit() == True for i in clened_cpf)


def test_correct_cpf_length():
    assert validate_cpf.__has_correct_length(VALID_CPF_WITHOUT_MASK) == True


def test_incorrect_cpf_length():
    SMALLER_CPF = '123456789'
    assert validate_cpf.__has_correct_length(SMALLER_CPF) == False
    BIGGER_CPF = '1234567891011'
    assert validate_cpf.__has_correct_length(BIGGER_CPF) == False


def test_is_not_allowed_cpf():
    ALLOWED_CPF = '11111111111'
    assert validate_cpf.__is_allowed(ALLOWED_CPF) == False


def test_is_allowed_cpf():
    assert validate_cpf.__is_allowed(VALID_CPF_WITHOUT_MASK) == True


def test_fist_verification_digit():
    CORRECT_FIST_DIGIT = int(VALID_CPF_WITHOUT_MASK[-2])
    assert validate_cpf.__caculate_digit(VALID_CPF_WITHOUT_MASK) == CORRECT_FIST_DIGIT


def test_last_verification_digit():
    CORRECT_LAST_DIGIT = int(VALID_CPF_WITHOUT_MASK[-1])
    assert (
        validate_cpf.__caculate_digit(VALID_CPF_WITHOUT_MASK, is_first_digit=False)
        == CORRECT_LAST_DIGIT
    )
